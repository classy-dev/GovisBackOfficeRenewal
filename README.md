...
기존 데이터 분석 시스템들은 대부분 백엔드 서버에서 무거운 SQL 처리를 수행하거나 사용자에게 직접 SQL 작성을 요구하는 한계가 있었습니다. 이를 극복하기 위해 웹어셈블리 기반 SQL 엔진을 프론트엔드에 구현하고, 대규모 언어 모델(LLM)과 연동하여 사용자가 자연어만으로 복잡한 데이터 분석을 수행할 수 있는 혁신적인 시스템을 개발했습니다.

#### 핵심 기능

- **데이터 저장 방식**: : Django 백엔드는 선택된 날짜 기반으로 메뉴 판매 데이터를 조회합니다. 프론트엔드에서 WebAssembly 기반 SQL.js(웹에서 실행되는 SQLite)로 데이터를 로드하고 브라우저의 IndexedDB에 저장합니다. <br><br>
- **자연어 질문 처리**: 사용자가 일상 언어로 질문을 입력합니다. (예: "어떤 매장이 피자를 가장 많이 팔았나요?") Anthropic API(Claude)가 자연어를 분석하고 SQL 쿼리로 변환합니다.<br><br>
- **브라우저 내 SQL 처리**: WebAssembly로 컴파일된 SQL.js가 프론트엔드에서 직접 쿼리를 실행하여 결과를 얻습니다.<br><br>
- **데이터 해석**: 얻은 결과를 다시 AI가 분석하여 자연어 응답과 차트/테이블로 변환합니다. 사용자에게 시각화된 데이터와 자연어 응답을 함께 제시합니다.

#### 기술적 특징

- **WebAssembly 으로 성능 최적화**: 일반 JavaScript만으로 SQL 엔진을 구현했다면 복잡한 쿼리 실행 시 성능이 현저히 떨어졌을 것입니다. 웹어셈블리는 네이티브에 가까운 속도를 제공하여 수만에서 수십만개의 레코드 분석 시에도 JavaScript 구현 대비 최대 20배 빠른 응답이 가능합니다.<br><br>
- **전체 SQLite 기능 사용**:웹어셈블리 덕분에 실제 SQLite의 모든 기능(복잡한 집계, 윈도우 함수, 조인 등)을 브라우저에서 사용할 수 있습니다. 일반 JavaScript로 구현된 SQL 엔진은 기능이 제한적이라는 한계가 있습니다.<br><br>
- **IndexedDB와의 관계**: IndexedDB는 SQL.js 데이터베이스 파일 자체를 저장하기 위한 용도로 사용됩니다. SQL.js는 실제 SQL 실행 엔진으로, 두 기술은 서로 다른 역할을 담당하며 상보적으로 동작하여 브라우저에서 완전한 SQLite 경험을 제공합니다.사용 가능
